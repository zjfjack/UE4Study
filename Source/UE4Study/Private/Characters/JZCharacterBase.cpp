...
void AJZCharacterBase::TraceFoward_Implementation()
{
    ...
    if (bHit)
        OnLineTraceHitActor(Hit.GetActor());
}

void AJZCharacterBase::Tick(float DeltaTime)
{
...
void AJZCharacterBase::OnLineTraceHitActor(AActor* Interactable)
{
...
void AJZCharacterBase::OnInteract(AActor* Actor)
{
    if (auto Interface = Cast<IInteractInterface>(Actor))
    {
        Interface->Execute_OnInteract(Actor, this);
    }
}

void AJZCharacterBase::StartFocus(AActor* Actor)
{
    if (auto Interface = Cast<IInteractInterface>(Actor))
    {
...
void AJZCharacterBase::EndFocus(AActor* Actor)
{
    if (auto Interface = Cast<IInteractInterface>(Actor))
    {
